...
## 实训简介
HCIA 云 & 网络综合实操实训，基于阿里云公有云资源搭建三层 Web 业务架构：应用型负载均衡 ALB → ECS (Linux+Nginx+PHP-FPM) → RDS MySQL，完成 WordPress 个人博客完整部署，覆盖云服务器、云数据库、负载均衡全流程配置、页面故障排查与链路验收。
## 技术栈
...
## RDS 自建数据库 vs 云数据库对比
   |对比项	|ECS 自建 MySQL|	RDS 托管云数据库|
   |安装维护|	手动安装、手动打安全补丁	|阿里云托管，自动升级内核 / 补丁|
   |备份恢复	|自行编写定时备份脚本	|控制台一键备份、一键回滚恢复|
   |高可用	|手动搭建主从架构，成本高|	自带主备双实例，故障自动切换|
   |监控告警	|自行搭建监控面板、配置告警|	内置 CPU / 连接数 / 磁盘监控，短信告警|
   |扩容性能|	扩容需停机，操作繁琐	|弹性升降配，分钟级完成，无停机|
...
bash
运行
# 错误操作：曾误将文件属主改为apache，PHP进程无读取权限
# 标准修复命令
...
# ✅ 学习成果
理解交换机、VLAN、MAC 地址表基础网络原理；
掌握 RDS 云数据库与自建 MySQL 的差异、托管数据库优势；
掌握 LNMP 架构组件分工、Linux 文件权限与 Web 进程用户匹配逻辑；
独立完成 RDS 实例创建、内网白名单、远程数据库连接配置；
熟练部署 WordPress 站点，独立编写数据库配置文件；
能定位并解决 WordPress 页面循环跳转、文件权限类故障；
掌握阿里云 ALB 负载均衡完整配置流程：实例、后端组、监听、健康检查；
可独立完成三层 Web 业务链路部署、全链路连通性验收测试。
